...
<c:if test="${action == 'update'}">
	...
	<h3>Forma parte de la pieza: ${weld.piece.name} (id
		nº${weld.piece.id})</h3>
</c:if>
...
<c:choose>
	<c:when test="${weld.state != 'CREADA'}">
		...
		<c:set var="mensaje" value="Valores de la soldadura"></c:set>
	</c:when>
	<c:otherwise>
...
<c:if test="${weld.state == 'INICIADA'}">
	<h3>
		<span class="badge bg-warning text-dark mb-3">Soldadura ya iniciada, no es posible modificar valores</span>
	</h3>
</c:if>
<h3>
	<span class="badge bg-danger mb-3">${weld.alarm.info}</span>
</h3>
<div class="card mt-1">
		...
		<form:form method="POST" action="/guardarSoldadura"
			...
			<table>
				...
				<tr>
					...
					<td><form:input path="amp" class="form-control mb-3"
							required="required" disabled="${disable}" /> <form:errors
							path="amp" class="error text-danger" /></td>

				</tr>
				<tr>
					<td><form:label path="volt" class="mb-3">Voltaje</form:label></td>
					<td><form:input path="volt" class="form-control mb-3"
							required="required" disabled="${disable}" /> <form:errors
							path="volt" class="error text-danger" /></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td><form:input path="note" class="form-control mb-3"
							disabled="${disable}" /></td>
				</tr>
				<tr>
