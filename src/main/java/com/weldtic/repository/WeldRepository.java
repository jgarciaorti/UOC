...
public interface WeldRepository<T extends Weld>extends JpaRepository<T, Long> {
	...
	@Query(value="select * from weld w join piece p on w.piece_id = p.id join project pr on  p.project_machine_project_id = pr.id join alarm a on a.weld_id = w.id	where pr.manager_id = ?1 and w.state='PENDIENTE'", nativeQuery = true)
	List<Weld> findByManagerWithAlarmPendant (Long id_manager);
}
